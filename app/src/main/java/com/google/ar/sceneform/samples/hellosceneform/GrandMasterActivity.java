...
public class GrandMasterActivity extends AppCompatActivity {
    ...
    private int maxY=3;
    private int maxZ=2;
    private Node[][][] arNodes = new  Node[maxX][maxY][maxZ];
    private Timer timer;
    private int randomX1;
    private int randomY1;
    private int randomZ1;
    private int randomX2;
    private int randomY2;
    private int randomZ2;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ModelRenderable.builder()
                ...
                .exceptionally(
                        throwable -> {
                            Toast toast =
                                    Toast.makeText(GrandMasterActivity.this, "Unable to load andy renderable", Toast.LENGTH_LONG);
                            toast.setGravity(Gravity.CENTER, 0, 0);
                            toast.show();
                            return null;
                        });
                                ...
                                node.setOnTapListener(new Node.OnTapListener() {
                                    ...
                                    public void onTap(HitTestResult hitTestResult, MotionEvent motionEvent) {
                                        ...
                                        if (nodeTapped != null) {
                                            ...
                                            int currentZ = Integer.parseInt(split[2]);
                                            if ((currentX == randomX1 && currentY == randomY1 && currentZ == randomZ1) || (currentX == randomX2 && currentY == randomY2&& currentZ == randomZ2)) {
                                                node.setRenderable(hamsterRenderableSchischk);
                                                Toast.makeText(GrandMasterActivity.this, "ты попал", Toast.LENGTH_SHORT).show();
                                                counter++;
                                                if (vibrateSwitchIsChecked) {
                                                    vibrate.vibrate(50);
                                                }
    ...
    private class MyTimerTask extends TimerTask{
        ...
        public void run() {
            ...
            Node nodePrev1 = arNodes[randomX1][randomY1][randomZ1];
            Node nodePrev2 = arNodes[randomX2][randomY2][randomZ2];

            Random random = new Random();
            randomX1 =random.nextInt(maxX);
            randomY1 =random.nextInt(maxY);
            randomZ1=random.nextInt(maxZ);
            randomX2 =random.nextInt(maxX);
            randomY2 =random.nextInt(maxY);
            randomZ2=random.nextInt(maxZ);
            Log.d("4343", randomX1 +" "+ randomY1);
            Node nodeNext1 = arNodes[randomX1][randomY1][randomZ1];
            Log.d("4343", randomX2 +" "+ randomY2);
            Node nodeNext2 = arNodes[randomX2][randomY2][randomZ2];
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    nodeNext1.setName(randomX1+";"+randomY1+";"+randomZ1);
                    nodeNext2.setName(randomX2+";"+randomY2+";"+randomZ2);
                }
            });
